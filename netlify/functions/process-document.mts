...
export default async (request: Request, context: Context) => {
  ...
  try {
    const notionResponse = await fetch(`${new URL(request.url).origin}/api/notion/${documentId}`, {
    ...
    const processResponse = await fetch(`${new URL(request.url).origin}/api/process-document-background`, {
      method: 'POST',
      headers: {
